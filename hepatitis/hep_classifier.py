...
if not os.path.exists('./hep_output'):
        os.mkdir('./hep_output')

#importing data
data = dataset.load_hepatitis() #directly importing from UCI repository
data=data.dropna()
data=data.dropna(axis=0)
data=data.dropna().reset_index(drop=True)

#raw data had the labels as '1' and '2'
...
df.to_csv(f'./hep_output/hep_score_copies_{max_copies}.csv')  

df = pd.read_csv(f'./hep_output/hep_score_copies_{max_copies}.csv')
...
ax.set_ylabel('Score')
plt.legend()
plt.savefig(f'./hep_output/score_test_{test_class}_{max_copies}.png')
plt.savefig(f'./hep_output/score_test_{test_class}_{max_copies}.pdf')
plt.show()
